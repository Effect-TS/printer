...
function nextToken<A>(): Parser<DocStream<A>, DocToken<A>> {
  return (stream) => {
    switch (stream._tag) {
      ...
      case "CharStream": {
        return Maybe.some(Tuple(DocToken.char(stream.char), stream.stream))
      }
      case "TextStream": {
        return Maybe.some(Tuple(DocToken.text(stream.text), stream.stream))
      }
      case "LineStream": {
        return Maybe.some(Tuple(DocToken.line(stream.indentation), stream.stream))
      }
      ...
      case "PopAnnotationStream": {
        return Maybe.some(Tuple(DocToken.popAnnotation, stream.stream))
      }
    }
  }
}
